...
> 실행용 초안·템플릿·GitHub 메타·UTM = **[`GROWTH_ASSETS.md`](GROWTH_ASSETS.md)** 에 정리됨(실탄).
1. **GitHub 저장소 메타 정비** — description·topics(복붙문구 준비됨) / OG 이미지(개발 필요).
2. ✅ **시드 글 초안** — 아카(자작)·디씨(밈)·레딧(EN)·개발기 4종 초안 완료 → GROWTH_ASSETS.
3. **스샷 세트 준비** — 시작화면·대화·엔딩(great/fail) → 시딩 첨부용.
4. **엔딩 공유 카드**(성장 루프 엔진) — 게임 재개 시 최우선 개발 항목.

> 미정 2개(AI 훅·공개 톤)는 시드 글 톤으로 자연스럽게 확정(초안은 "내세우되 겸손" 기본값).
