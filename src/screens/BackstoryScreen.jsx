    ...
    <div style={{minHeight:"100dvh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",position:"relative",overflowY:"auto",WebkitOverflowScrolling:"touch",fontFamily:"'Noto Sans KR',sans-serif"}}>
      ...
      <div style={{width:"100%",maxWidth:500,padding:"48px 22px 32px",opacity:vis?1:0,transform:vis?"translateY(0)":"translateY(14px)",transition:"all 0.28s cubic-bezier(.4,0,.2,1)",position:"relative",zIndex:1}}>
        ...
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{fontSize:9,letterSpacing:8,color:"rgba(212,169,96,0.5)",fontFamily:"monospace",marginBottom:8}}>PROLOGUE</div>
          <h1 style={{fontSize:30,fontWeight:800,color:"#d4a960",lineHeight:1.15,fontFamily:"'Nanum Myeongjo',serif",letterSpacing:1,textShadow:"0 2px 20px rgba(212,169,96,0.25)"}}>준모의 소개팅</h1>
        </div>

        {/* 스텝 도트 */}
        <div style={{display:"flex",justifyContent:"center",gap:5,marginBottom:22}}>
          {BACKSTORY.map((_,i)=>(
            <div key={i} style={{width:i===sc?22:6,height:5,borderRadius:3,background:i===sc?"#d4a960":i<sc?"rgba(212,169,96,0.4)":"rgba(255,255,255,0.12)",transition:"all 0.35s ease",boxShadow:i===sc?"0 0 8px rgba(212,169,96,0.5)":""}}/>
          ))}
        </div>
        ...
        <div style={{background:"rgba(15,10,6,0.78)",border:"1px solid rgba(255,200,140,0.12)",borderRadius:22,padding:"26px 26px 22px",marginBottom:14,backdropFilter:"blur(28px)",position:"relative",overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,0.6)"}}>
          <div style={{position:"absolute",top:0,left:"15%",right:"15%",height:1,background:"linear-gradient(90deg,transparent,rgba(212,169,96,0.45),transparent)"}}/>
          {/* 상단: 아이콘 + 연도 */}
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:18}}>
            <div style={{width:52,height:52,borderRadius:14,background:"rgba(212,169,96,0.08)",border:"1px solid rgba(212,169,96,0.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,flexShrink:0}}>{s.icon}</div>
            <div>
              <div style={{fontSize:8,color:"rgba(212,169,96,0.55)",letterSpacing:4,fontFamily:"monospace",marginBottom:4}}>{s.yr}</div>
          ...
          {/* 인용 */}
          <div style={{background:"rgba(212,169,96,0.04)",border:"1px solid rgba(212,169,96,0.1)",borderLeft:"3px solid rgba(212,169,96,0.35)",borderRadius:"0 10px 10px 0",padding:"12px 16px"}}>
        ...
        <button onClick={next}
          style={{width:"100%",padding:"14px",
            background: sc===BACKSTORY.length-1
              ? "linear-gradient(135deg,#e0b877,#c8974a)"
              : "rgba(255,255,255,0.05)",
            border: sc===BACKSTORY.length-1 ? "none" : "1px solid rgba(255,255,255,0.1)",
            borderRadius:14,color: sc===BACKSTORY.length-1 ? "#1a1410" : "white",fontWeight:800,fontSize:14,cursor:"pointer",
            fontFamily:"'Noto Sans KR',sans-serif",transition:"all 0.2s",
            boxShadow: sc===BACKSTORY.length-1 ? "0 8px 28px rgba(212,169,96,0.35)" : "none"}}
